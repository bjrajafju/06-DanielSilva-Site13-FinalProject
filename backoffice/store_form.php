...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        $store_id = $id;
        set_alert("Loja atualizada com sucesso!");
    } else {
        $data['created_at'] = date('Y-m-d H:i:s');
        $store_id = db_insert('stores', $data);
        set_alert("Loja criada com sucesso!");
    }
...
<div id="content">
    <div class="topbar">
        <h2 class="h4 mb-0"><?= $id ? 'Editar Loja' : 'Criar Loja' ?></h2>
        <a href="stores.php" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left"></i> Voltar à Lista</a>
    </div>
            ...
            <div class="col-lg-8">
                <form action="" method="POST">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="custom-control custom-switch mb-2">
                                <input class="custom-control-input" type="checkbox" name="is_active" id="isActive" <?= ($store['is_active'] ?? 1) ? 'checked' : '' ?>>
                            ...
                            <div class="tab-content">
                                ...
                                ?>
                                    <div class="tab-pane fade <?= $index === 0 ? 'show active' : '' ?>" id="content-<?= $lang['code'] ?>" role="tabpanel">
                                        <div class="mb-3">
                                            <label class="form-label">Nome da Loja (<?= strtoupper($lang['code']) ?>)</label>
                                            <input type="text" name="trans[<?= $lang['code'] ?>][name]" class="form-control" value="<?= $t['name'] ?? '' ?>" required>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Morada (<?= strtoupper($lang['code']) ?>)</label>
                    ...
                    <button type="submit" class="btn btn-primary w-100">
                        <i class="fas fa-save"></i> Guardar Loja
                    </button>
                </form>
            </div>
        </div>
