...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        $product_id = $id;
        set_alert("Produto atualizado com sucesso!");
    } else {
...
<div id="content">
    <div class="topbar">
        <h2 class="h4 mb-0"><?= $id ? 'Editar Produto' : 'Criar Produto' ?></h2>
        <a href="products.php" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left"></i> Voltar à Lista</a>
    </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="codProd" class="form-control"
                                        value="<?= $product['codProd'] ?? '' ?>" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                ...
                                <?php foreach ($languages as $index => $lang): ?>
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link <?= $index === 0 ? 'active' : '' ?>"
                                            ...
                                            data-target="#content-<?= $lang['code'] ?>" type="button" role="tab">
                                ...
                                <?php foreach ($languages as $index => $lang):
                                    ...
                                    <div class="tab-pane fade <?= $index === 0 ? 'show active' : '' ?>"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="trans[<?= $lang['code'] ?>][title]"
                                                class="form-control" value="<?= $t['title'] ?? '' ?>" required>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Slug (<?= strtoupper($lang['code']) ?>) - Deixar vazio
                                                para gerar auto.</label>
                                            <input type="text" name="trans[<?= $lang['code'] ?>][slug]" class="form-control"
                                                value="<?= $t['slug'] ?? '' ?>">
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Descrição Curta</label>
                                            <textarea name="trans[<?= $lang['code'] ?>][short_description]"
                                                class="form-control"
                                                rows="2"><?= $t['short_description'] ?? '' ?></textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Descrição Completa</label>
                                            <textarea name="trans[<?= $lang['code'] ?>][description]" class="form-control"
                                                rows="5"><?= $t['description'] ?? '' ?></textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Informação Adicional</label>
                                            <textarea name="trans[<?= $lang['code'] ?>][additional_info]"
                                                class="form-control" rows="3"><?= $t['additional_info'] ?? '' ?></textarea>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body text-center">
                            <div class="mb-3">
                                ...
                                <div class="form-text">Carregar imagem do produto (PNG, JPG)</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <a href="products.php" class="btn btn-outline-secondary w-100">Cancelar</a>
                        </div>
                    </div>
                </div>
            </div>
